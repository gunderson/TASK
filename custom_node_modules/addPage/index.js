...
function makePUG( name ) {
	...
	Fs.writeFileSync( Path.resolve( __dirname, '../..', 'src/pug/static/pages', `_${name}.pug` ), rendered );
	return rendered;
}
...
function makeSASS( name ) {
	...
	Fs.writeFileSync( Path.resolve( __dirname, '../..', 'src/sass/pages', `_${name}.sass` ), rendered );
	return rendered;
}
...
function addToPugIndex( name ) {
	let importsMarker = '// imports\n';
	let srcPath = Path.resolve( __dirname, '../..', 'src/js/static/index.pug' );
	let fileContents = Fs.readFileSync( srcPath, 'utf-8' );
	let parts = fileContents.split( importsMarker );
