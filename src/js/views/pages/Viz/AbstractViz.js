...
var Visualizer = Backbone.View.extend( {
	initialize: setup,
	update: update,
	render: render,
	reset: reset,
...
function setup( options ) {
	...
	camera = new THREE.PerspectiveCamera(
		this.controls.viewAngle, // view angle
		this.controls.canvasWidth / this.controls.canvasHeight, // aspect
		0.01, // near
		10000 // far
	);
	camera.position = this.controls.cameraPosition;
	camera.lookAt( this.controls.cameraFocus );
	...
	standardGeometry = new THREE.CylinderGeometry(
		...
		this.controls.geometryWidth, // lower radius
		this.controls.geometryHeight, // height
		this.controls.geometrySides // segments
	);

	standardMaterial = new THREE.MeshLambertMaterial( {
		color: 0xffffff,
		transparent: true,
	...
	// prep the renderer
	renderer.setSize( this.controls.canvasWidth, this.controls.canvasHeight );
...
var currentCameraViewOffset = 0;

function nextCameraView() {
	currentCameraViewOffset = ( currentCameraViewOffset + 1 ) % cameraViewOffsets.length;
	camera.setViewOffset(
		renderer.domElement.width,
		renderer.domElement.height,
		cameraViewOffsets[ currentCameraViewOffset ].x,
		cameraViewOffsets[ currentCameraViewOffset ].y,
		cameraViewOffsets[ currentCameraViewOffset ].width,
		cameraViewOffsets[ currentCameraViewOffset ].height
	);
}
